...
<head>
    ...
    <title>Eliminar Clientes - EPS Securitas</title>
    <!-- Bootstrap CSS -->
...
<body>
    <div class="container">
        <h1 class="mt-5 mb-4">Eliminar Clientes</h1>
        ...
        <% 
            if ("POST".equals(request.getMethod())) {
                // Procesar el formulario para eliminar el cliente
                ...
                try {
                    ...
                    pstmtEliminar.setString(1, documento);
                    
                    int filasEliminadas = pstmtEliminar.executeUpdate();
                    
                    if (filasEliminadas > 0) {
        %>
        <div class="alert alert-success mt-3">Cliente eliminado exitosamente</div>
        <%              } else { %>
        <div class="alert alert-danger mt-3">Error al eliminar el cliente</div>
        <%              }
                } catch (ClassNotFoundException | SQLException e) {
                    e.printStackTrace();
                } finally {
                    try {
                        if (pstmtEliminar != null) pstmtEliminar.close();
                        if (conexionEliminar != null) conexionEliminar.close();
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                }
